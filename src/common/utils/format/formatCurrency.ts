...
function format(amount: number | bigint, showAbbr: boolean): string {
  if (showAbbr) {
    ...
    if (amount >= TRILLION) {
      return formatWithAbbr(amount, TRILLION, 'T');
    }
  }
...
function formatSmallCurrency(amount: number) {
  ...
  let minimalDigits = 0;
  for (; Number(amount) % 10 ** minimalDigits === 0; ) {
    minimalDigits = minimalDigits + 1;
  }

  // if both last 2 digits are 0
  if (
    Number(amount) % 10 ** (10 - (fixNum + 2)) === 0 &&
    Number(amount) % 10 ** (10 - (fixNum + 1)) === 0
  ) {
...
function formatCurrencyBigInt(
  ...
  // @ts-expect-error ignore
  if (amount < 0n) {
    return `-${formatCurrencyBigInt(-amount, options)}`;
  }
